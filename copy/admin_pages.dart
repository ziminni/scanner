...
class DashboardPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      padding: _pagePadding(systemAdmin),
      children: [
        _PageTitle(title: 'Dashboard', systemAdmin: systemAdmin),
        ...
        GridView(
          ...
          gridDelegate: SliverGridDelegateWithMaxCrossAxisExtent(
            ...
            mainAxisExtent: systemAdmin ? 116 : 104,
            crossAxisSpacing: systemAdmin ? 16 : 12,
            mainAxisSpacing: systemAdmin ? 16 : 12,
          ),
          children: [
            if (systemAdmin)
              FirestoreCount(
        ...
        Text(
          ...
          style: Theme.of(context).textTheme.titleLarge?.copyWith(
            fontWeight: systemAdmin ? FontWeight.w600 : null,
            color: systemAdmin ? AppColors.adminText : null,
          ),
        ),
            ...
            child: DataTable(
              border: TableBorder(
                horizontalInside: BorderSide(
                  color: AppColors.adminBorder.withValues(alpha: 0.75),
                  width: 0.8,
                ),
              ),
              columnSpacing: 24,
              ...
              rows: [
                for (final user in users)
                  DataRow(
                    cells: [
                      DataCell(_TableText(user.fullName, maxWidth: 240)),
                      DataCell(_TableText(user.email, maxWidth: 260)),
                      DataCell(_TableText(user.role.label, maxWidth: 160)),
                      DataCell(_TableText(user.status.label, maxWidth: 120)),
                      DataCell(
                        _TableText(
                          ...
                          children: [
                            _TableActionButton(
                              tooltip: user.status == AccountStatus.active
                                  ? 'Disable account'
                                  : 'Enable account',
                              icon: user.status == AccountStatus.active
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        // Responsive column widths to reduce clipping on small screens
        final widths = [100.0, 220.0, 120.0, 120.0, 180.0, 120.0, 120.0, 140.0, 100.0];
        return _DataSurface(
          child: SingleChildScrollView(
            scrollDirection: Axis.horizontal,
            child: DataTable(
              ...
              rows: [
                for (final log in logs)
                  DataRow(
                    cells: [
                      DataCell(_TableText(log.personId, maxWidth: widths[0])),
                      DataCell(_TableText(log.fullName, maxWidth: widths[1])),
                      DataCell(_TableText(log.personRole.label, maxWidth: widths[2])),
                      DataCell(_TableText(log.section, maxWidth: widths[3])),
                      DataCell(_TableText('${log.dateKey} ${log.timeText}', maxWidth: widths[4])),
                      DataCell(_TableText(log.attendanceType.label, maxWidth: widths[5])),
                      DataCell(_TableText(log.attendanceStatus.label, maxWidth: widths[6])),
                      DataCell(_TableText(log.scannedBy, maxWidth: widths[7])),
                      DataCell(_TableText(log.syncStatus.label, maxWidth: widths[8])),
                    ],
                  ),
              ],
            ),
...
class _Page extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final systemAdmin =
        AppScope.of(context).currentUser?.role == UserRole.systemAdministrator;
    return ListView(
      padding: _pagePadding(systemAdmin),
      children: [
        Wrap(
          ...
          runSpacing: 12,
          children: [
            _PageTitle(title: title, systemAdmin: systemAdmin),
            Wrap(spacing: 8, runSpacing: 8, children: actions),
          ],
        ),
        SizedBox(height: systemAdmin ? 18 : 16),
        child,
      ],
    );
  }
...
class _PageTitle extends StatelessWidget {
  const _PageTitle({required this.title, required this.systemAdmin});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: systemAdmin ? AppColors.adminSurface : null,
        border: systemAdmin
            ? Border.all(color: AppColors.adminAccent.withValues(alpha: 0.12))
            : Border.all(color: Theme.of(context).colorScheme.outlineVariant),
        borderRadius: BorderRadius.circular(systemAdmin ? 18 : 8),
        boxShadow: systemAdmin
            ? [
                BoxShadow(
...
class _TableActionButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Tooltip(
      message: tooltip,
      child: Material(
        color: AppColors.adminPrimary.withValues(alpha: 0.32),
        borderRadius: BorderRadius.circular(10),
        child: InkWell(
          borderRadius: BorderRadius.circular(10),
          hoverColor: AppColors.adminAccent.withValues(alpha: 0.10),
          onTap: onPressed,
          child: Padding(
...
EdgeInsets _pagePadding(bool systemAdmin) {
  return systemAdmin
